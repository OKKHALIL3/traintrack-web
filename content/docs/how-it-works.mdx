...
Headless turns are wired only for `claude` and `codex` (the two beta agents). The other supported CLIs — Cursor, OpenCode, Windsurf, Cline, Kiro, Zed, Continue, and GitHub Copilot CLI — join the mesh and get `/team` (alpha), but cannot yet be spawned as headless workers. The supported values for `agent` here are `claude` and `codex`.
...
The verification scripts live at the repo root. They are not test fixtures — they prove the full path from spawn through reply to collection.

| Script | What it proves |
| --- | --- |
| `verify-l1.mjs` | Spawn a real codex worker, collect a `PONG` reply. |
