...
> **Note.** Only Claude Code and Codex are **beta** — live-verified end-to-end (spawn → work → collect) with real agents. The other eight harnesses are **alpha**: wired per each host's official MCP + command docs, but not yet live-verified. Check the output summary carefully.
...
| Flag | Required | Description |
|---|---|---|
| `--agent` | Yes | Agent to run. Must be exactly `claude` or `codex`. |
...
| `--agent` | No | Agent to run. Must be `claude` or `codex`. Defaults to `codex`. |
