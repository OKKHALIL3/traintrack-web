...
> **Alpha.** Commands, config formats, and the wire protocol can change between versions. Claude Code and Codex are **beta** — live-verified end-to-end. The other eight CLIs (Cursor, OpenCode, Windsurf, Cline, Kiro, Zed, Continue, and GitHub Copilot CLI) are **alpha** — wired per each host's official docs but not yet live-verified. Pin a version if you depend on it.
